    ...
    with tf.contrib.slim.arg_scope(resnet_v2.resnet_arg_scope(batch_norm_decay=batch_norm_decay)):
      with arg_scope([layers.batch_norm], is_training=is_training):
        ...
        #with tf.variable_scope("image_level_features"):
          ...
          #image_level_features_2048 = tf.reduce_mean(inputs, [1, 2], name='global_average_pooling', keepdims=True)
          # 1x1 convolution with 256 filters( and batch normalization)
          #image_level_features_256 = layers_lib.conv2d(image_level_features_2048, depth, [1, 1], stride=1, scope='conv_1x1')
          # bilinearly upsample features
          #image_level_features = tf.image.resize_bilinear(image_level_features_256, inputs_size, name='upsample')


  with tf.variable_scope("vlad"):
    centers = np.load(netvlad_centers)['centers'].T
    vlad_features = netvlad.vlad_pooling_layer(inputs=conv_1x1, K=netvlad_k, centers=centers)
    n_flat = vlad_features.get_shape().as_list()[1]
    weights = tf.Variable(tf.truncated_normal(shape=[n_flat, 256], mean=0.0, stddev=0.01), name='weights')
    biases = tf.Variable(tf.constant(0.01, shape=[256]), name='biases')
    vlad_features_256 = tf.matmul(vlad_features, weights)
    vlad_features_256 = tf.nn.bias_add(vlad_features_256, biases, name='fc')
    vlad_features_256 = tf.expand_dims(vlad_features_256, axis=1)
    vlad_features_256 = tf.expand_dims(vlad_features_256, axis=1)
    vlad_features = tf.image.resize_bilinear(vlad_features_256, inputs_size, name='upsample')


  with tf.variable_scope("decoder"):
    net_concat = tf.concat([conv_1x1, conv_3x3_1, conv_3x3_2, conv_3x3_3, vlad_features], axis=3, name='concat')
    net = layers_lib.conv2d(net_concat, depth, [1, 1], stride=1, scope='conv_1x1_concat')

#        net_concat = tf.concat([conv_1x1, conv_3x3_1, conv_3x3_2, conv_3x3_3, image_level_features], axis=3, name='concat')
#        net = layers_lib.conv2d(net_concat, depth, [1, 1], stride=1, scope='conv_1x1_concat')

  debug_tensors['conv_1x1'] = conv_1x1
  debug_tensors['net_concat'] = net_concat
  #debug_tensors['image_level_features_256'] = image_level_features_256
  return net, debug_tensors
...
def deeplab_v3_plus_generator(num_classes,
  ...
  def model(inputs, is_training):
    ...
    if is_training:
      ...
      print('-----------------------------------------------')
      print('loading checkpoint', pre_trained_model, '...')
    ...
    encoder_output, debug_tensors = atrous_spatial_pyramid_pooling(net, 
                                                                   output_stride, 
                                                                   batch_norm_decay, 
                                                                   is_training,
                                                                   netvlad_k, 
    ...
    debug_tensors['low_level_features'] = low_level_features
...
def deeplabv3_plus_model_fn(features, labels, mode, params):
  ...
  network = deeplab_v3_plus_generator(params['num_classes'],
                                      ...
                                      params['Netvlad_K'],
                                      params['Netvlad_centers'])
  logits, debug_tensors = network(features, mode == tf.estimator.ModeKeys.TRAIN) #[b, 285, 285, 21]
...
#                     max_outputs=params['tensorboard_images_max_outputs'])  # Concatenate row-wise.
    ...
    encoder_trainable = [v for v in train_var_list if 'resnet' in v.name]
    decoder_aspp_trainable = [v for v in train_var_list if 'aspp' in v.name or 'decoder' in v.name]
    decoder_vlad_trainable = [v for v in train_var_list if 'vlad' in v.name]

    #assert(len(train_var_list) == len(encoder_trainable) + len(decoder_aspp_trainable))
    assert(len(train_var_list) == len(encoder_trainable) + len(decoder_aspp_trainable) + len(decoder_vlad_trainable))
    ...
    opt_decoder_vlad = tf.train.MomentumOptimizer(learning_rate=learning_rate * 1, momentum=params['momentum'])

    #grads = tf.gradients(loss, encoder_trainable + decoder_aspp_trainable)
    grads = tf.gradients(loss, encoder_trainable + decoder_aspp_trainable + decoder_vlad_trainable)
    ...
    grads_decoder_vlad = grads[(len(encoder_trainable) + len(decoder_aspp_trainable)):]
    ...
    train_op_decoder_vlad = opt_decoder_vlad.apply_gradients(zip(grads_decoder_vlad, decoder_vlad_trainable), global_step=global_step)
    ...
    with tf.control_dependencies(update_ops):
      ...
      train_op = tf.group(train_op_encoder, train_op_decoder_aspp, train_op_decoder_vlad)
  else:
    train_op = None
